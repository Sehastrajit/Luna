...
export class LunaChatViewProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'luna.chatView';
    public static readonly viewTypeSecondary = 'luna.chatViewSecondary';

    private _view?: vscode.WebviewView;
    private _conversationId?: number;
    private _codeHistory: Array<{ role: string; content: string }> = [];
    ...
    private async _streamChat(userText: string): Promise<void> {
        const config = vscode.workspace.getConfiguration('luna');
        const baseUrl = config.get<string>('backendUrl', 'http://localhost:8765');
        ...
        await this._doRequest(`${baseUrl}/api/chat/stream`, body, (data) => {
            if (data.type === 'meta' && data.conversation_id) {
                this._conversationId = data.conversation_id as number;
    ...
    private async _streamCode(userText: string): Promise<void> {
        const config = vscode.workspace.getConfiguration('luna');
        const baseUrl = config.get<string>('backendUrl', 'http://localhost:8765');
    ...
    private _insertIntoEditor(code: string): void {
        const editor = vscode.window.activeTextEditor;
    ...
    private async _applyFileEdit(filePath: string, content: string): Promise<void> {
        const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
    ...
    private async _readWorkspaceFile(filePath: string): Promise<string | null> {
